...
#SBATCH --job-name=b_trapr_v1                                # sets the job name
#SBATCH --output=outfiles/b_trapr_v1.out.%j                            # indicates a file to redirect STDOUT to; %j is the jobid 
#SBATCH --error=outfiles/b_trapr_v1.out.%j                             # indicates a file to redirect STDERR to; %j is the jobid
...
srun bash -c 'hostname; CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python ./tools/run_distributed_engines.py \
                ...
                config.MODEL.WEIGHTS_INIT.PARAMS_FILE=/vulcanscratch/mgwillia/trapr/trapr_v1_checkpoint.pth.tar \
                ...
                config.CHECKPOINT.DIR="/cfarhomes/mgwillia/vissl/checkpoints_bench_trapr_v1" config.DATA.TRAIN.BATCHSIZE_PER_REPLICA=32'
