...
#SBATCH --job-name=moco_d_inet
#SBATCH --output=outfiles/moco_d_inet.out.%j
#SBATCH --error=outfiles/moco_d_inet.out.%j
...
srun bash -c 'hostname; CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python ./tools/run_distributed_engines.py config=./pretrain/moco/moco_distill_1node_resnet \
    ...
    config.CHECKPOINT.DIR="/vulcanscratch/mgwillia/vissl/checkpoints_moco_d" config.DATA.TRAIN.BATCHSIZE_PER_REPLICA=32'
