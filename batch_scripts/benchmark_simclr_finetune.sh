...
#SBATCH --job-name=b_sclr_f_inet                                # sets the job name
#SBATCH --output=outfiles/b_sclr_f_inet.out.%j                            # indicates a file to redirect STDOUT to; %j is the jobid 
#SBATCH --error=outfiles/b_sclr_f_inet.out.%j                             # indicates a file to redirect STDERR to; %j is the jobid
...
srun bash -c 'hostname; CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python ./tools/run_distributed_engines.py \
                ...
                config.MODEL.WEIGHTS_INIT.PARAMS_FILE=checkpoints_sclr_f/model_final_checkpoint_phase9.torch \
                ...
                config.CHECKPOINT.DIR="/vulcanscratch/mgwillia/vissl/checkpoints_bench_simclr_finetune" config.DATA.TRAIN.BATCHSIZE_PER_REPLICA=32'
