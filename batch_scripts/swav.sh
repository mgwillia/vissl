...
#SBATCH --job-name=swav_inet                                # sets the job name
#SBATCH --output=outfiles/swav_inet.out.%j                            # indicates a file to redirect STDOUT to; %j is the jobid 
#SBATCH --error=outfiles/swav_inet.out.%j                             # indicates a file to redirect STDERR to; %j is the jobid
...
srun bash -c 'hostname; CUDA_VISIBLE_DEVICES=0,1,2,3 python ./tools/run_distributed_engines.py config=./pretrain/swav/swav_8node_resnet \
