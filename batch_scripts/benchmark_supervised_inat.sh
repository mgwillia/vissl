...
#SBATCH --job-name=b_sup_inat
#SBATCH --output=outfiles/b_sup_inat.out.%j
#SBATCH --error=outfiles/b_sup_inat.out.%j
...
if [ ! -d /scratch0/mgwillia/inat_comp_2021/val ]; then
    ...
    srun bash -c "./msrsync -p 16 -P -r /fs/vulcan-datasets/inat_comp_2021 /scratch0/mgwillia/"
fi
...
srun bash -c 'hostname; python ./tools/run_distributed_engines.py \
                ...
                config.MODEL.WEIGHTS_INIT.PARAMS_FILE=/vulcanscratch/mgwillia/unsupervised-classification/backbones/supervised_r50.torch \
                ...
                config.CHECKPOINT.DIR="/vulcanscratch/mgwillia/vissl/checkpoints_bench_sup_inat" config.DATA.TRAIN.BATCHSIZE_PER_REPLICA=64'
